...
from model.lossfunction import LayerSegLoss, LayerSegLoss_Pre, MaskedBCELoss, MaskedCELoss, MaskedMSELoss
from model.utils import *
from dataset import Data_Loader_Nonoverlap
...
class MyTrainModule(BaseTrainModule):
    def __init__(self):
        ...
        # self.save_name = '256_(1_4)_2_4'

        self.save_name = '256_(1_4)_2_5'  # without transformed
    ...
    def configure_lossfunctions(self):
        self.criterion_G = LayerSegLoss_Pre()
    ...
    def training_step(self, batch_idx, batch):
        ...
        image, layer_masks, layer_masks_crop, _, bone_layers_GT, image_nonoverlap, layer_masks_nonoverlap = batch

        context_mask = torch.clone(layer_masks[:, 2:, :, :])
        context_masks = context_mask.repeat(1, 3, 1, 1)

        context_mask_no = torch.clone(torch.sum(layer_masks_nonoverlap, dim=1)).unsqueeze(1)
        context_mask_no[context_mask_no != 0] = 1
        context_masks_no = context_mask_no.repeat(1, 3, 1, 1)

        # Segmenter training
        self.optimizer_S.zero_grad()

        pre_mask_overlap = self.net_S(image)
        pre_mask_nonoverlap = self.net_S(image_nonoverlap)

        loss_S = 0.5 * self.criterion_S(pre_mask_overlap, layer_masks_crop, context_masks) + 0.5 * self.criterion_S(
            pre_mask_nonoverlap, layer_masks_nonoverlap, context_masks_no)
        ...
        loss_G = self.criterion_G(org_image=image, org_masks=layer_masks, pre_layers=pre_layers,
                                  bone_layers_GT=bone_layers_GT,
                                  segmenter=self.net_S, discriminator=self.net_D, times=2)
    ...
    def validation_step(self, batch_idx, batch):
        ...
        loss_S_val = 0.5 * self.criterion_S(pre_mask_overlap, layer_masks_crop, context_masks) + 0.5 * self.criterion_S(
            pre_mask_nonoverlap, layer_masks_nonoverlap, context_masks_no)
        ...
        show_nonoverlap = torch.cat((image_nonoverlap, F.sigmoid(pre_mask_nonoverlap), F.sigmoid(real_labels)), dim=1)
    ...
    def configure_saveprocess(self):
        self.set_save_parameter(model=self.net_G, loss_name='Loss_G',
                                save_path='{}/parameter/1_4/generator_Pre_{}_{}_{}_{}.pth'.format(ROOT_PATH,
                                                                                                  self.save_name,
                                                                                                  self.generator_backbone,
                                                                                                  self.segmenter_backbone,
                                                                                                  self.discriminator_backbone))
        self.set_save_parameter(model=self.net_S, loss_name='Loss_S',
                                save_path='{}/parameter/1_4/segmenter_Pre_{}_{}_{}_{}.pth'.format(ROOT_PATH,
                                                                                                  self.save_name,
                                                                                                  self.generator_backbone,
                                                                                                  self.segmenter_backbone,
                                                                                                  self.discriminator_backbone))
        self.set_save_parameter(model=self.net_D, loss_name='Loss_D',
                                save_path='{}/parameter/1_4/discriminator_Pre_{}_{}_{}_{}.pth'.format(ROOT_PATH,
                                                                                                      self.save_name,
                                                                                                      self.generator_backbone,
                                                                                                      self.segmenter_backbone,
                                                                                                      self.discriminator_backbone))
...
if __name__ == "__main__":
    ...
    train_dataset = Data_Loader_Nonoverlap(ROOT_PATH + '/Data/LS_K1_nonoverlap_train', transform)
    valid_dataset = Data_Loader_Nonoverlap(ROOT_PATH + '/Data/LS_K1_nonoverlap_test', transform)
    ...
    trainer.configure(batch_size=9, epochs=150, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'),
                      result_number=2, lr_G=1e-4, lr_S=1e-4, lr_D=3e-4)
    trainer.fit()
