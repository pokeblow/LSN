...
def pocfunc_model(alpha, delta1, delta2, r, u):
    ...
    return lambda n1, n2: alpha / (N1 * N2) * np.sin((n1 + delta1) * V1 / N1 * np.pi) * np.sin(
        (n2 + delta2) * V2 / N2 * np.pi) \
...
def gullyDetect(img, maxWidth=7):
    ...
    horizon = img + cv2.warpAffine(img, np.float32([[1, 0, 1], [0, 1, 0]]), imgShape) / 2 + cv2.warpAffine(img,
                                                                                                           np.float32([[
                                                                                                                           1,
                                                                                                                           0,
                                                                                                                           -1],
